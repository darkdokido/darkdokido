...
usage：
    ...
    [早晚安设置] 查看配置
""".strip()
__plugin_des__ = "记录群友作息，帮助大家改善睡眠"
...
__plugin_cmd__ = ["早晚安", "早安", "晚安"]
__plugin_version__ = 2.2
__plugin_author__ = "MinatoAquaCrews"
__plugin_settings__ = {
    "level": 5,
    "default_status": True,
...
plugin_help = on_command("早晚安帮助", permission=GROUP, priority=5, block=True)
morning = on_command(
    "早安", aliases={"哦嗨哟", "おはよう"}, permission=GROUP, priority=5, block=True
)
night = on_command(
    "晚安", aliases={"哦呀斯密", "おやすみ"}, permission=GROUP, priority=5, block=True
)
# routine
my_routine = on_command("我的作息", permission=GROUP, priority=5, block=True)
fellow_routine = on_command("群友作息", permission=GROUP, priority=5, block=True)
# setting
setting = on_command("早晚安设置", permission=GROUP, priority=5, block=True)
morning_setting = on_command(
    "早安设置", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
morning_on = on_command(
    "早安开启", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
morning_off = on_command(
    "早安关闭", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
night_setting = on_command(
    "晚安设置", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
night_on = on_command(
    "晚安开启", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
night_off = on_command(
    "晚安关闭", permission=SUPERUSER | GROUP_OWNER | GROUP_ADMIN, priority=5, block=True
)
