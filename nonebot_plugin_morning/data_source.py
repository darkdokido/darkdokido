...
mor_switcher = {"时限": "get_up_intime", "多重起床": "multi_get_up", "超级亢奋": "super_get_up"}

nig_switcher = {"时限": "sleep_intime", "优质睡眠": "good_sleep", "深度睡眠": "deep_sleep"}
...
class MorningManager:
    ...
    def _init_data(self, group_id: str) -> None:
        if group_id not in self.user_data.keys():
            self.user_data[group_id] = {"today_count": {"morning": 0, "night": 0}}
    ...
    def get_current_config(self) -> str:
        ...
        if get_up_intime:
            msg = (
                msg
                + "\n是否要求规定时间内起床：是\n - 最早允许起床时间："
                + str(self.config["morning"]["get_up_intime"]["early_time"])
                + "点\n - 最晚允许起床时间："
                + str(self.config["morning"]["get_up_intime"]["late_time"])
                + "点"
            )
        else:
            msg = msg + "\n是否要求规定时间内起床：否"
        ...
        if multi_get_up:
            ...
            msg = (
                msg
                + "\n是否允许连续多次起床：否\n - 允许的最短起床间隔："
                + str(self.config["morning"]["multi_get_up"]["interval"])
                + "小时"
            )
        ...
        if super_get_up:
            ...
            msg = (
                msg
                + "\n是否允许超级亢奋(即睡眠时长很短)：否\n - 允许的最短睡觉时长："
                + str(self.config["morning"]["super_get_up"]["interval"])
                + "小时"
            )
        # night_config
        ...
        if sleep_intime:
            msg = (
                msg
                + "\n是否要求规定时间内睡觉：是\n - 最早允许睡觉时间："
                + str(self.config["night"]["sleep_intime"]["early_time"])
                + "点\n - 最晚允许睡觉时间：第二天早上"
                + str(self.config["night"]["sleep_intime"]["late_time"])
                + "点"
            )
        else:
            msg = msg + "\n是否要求规定时间内睡觉：否"
        ...
        if good_sleep:
            ...
            msg = (
                msg
                + "\n是否开启优质睡眠：否\n - 允许的最短优质睡眠："
                + str(self.config["night"]["good_sleep"]["interval"])
                + "小时"
            )
        ...
        if deep_sleep:
            ...
            msg = (
                msg
                + "\n是否允许深度睡眠(即清醒时长很短)：否\n - 允许的最短清醒时长："
                + str(self.config["night"]["deep_sleep"]["interval"])
                + "小时"
            )
        return msg
    ...
    @staticmethod
    def judge_mor_time(
        early_time_tmp: str, late_time_tmp: str, now_time: datetime.datetime
    ) -> bool:
        early_time = datetime.datetime.strptime(
            str(datetime.datetime.now().date()) + f" {early_time_tmp}:00:00",
            "%Y-%m-%d %H:%M:%S",
        )
        late_time = datetime.datetime.strptime(
            str(datetime.datetime.now().date()) + f" {late_time_tmp}:00:00",
            "%Y-%m-%d %H:%M:%S",
        )
    ...
    # 判断多次早安
    def judge_have_mor(
        self, group_id: str, user_id: str, now_time: datetime.datetime, interval: str
    ) -> bool:
        get_up_time = datetime.datetime.strptime(
            self.user_data[group_id][user_id]["get_up_time"], "%Y-%m-%d %H:%M:%S"
        )
    ...
    # 判断超级亢奋
    def judge_super_get_up(
        self, group_id: str, user_id: str, now_time: datetime.datetime, interval: str
    ) -> bool:
        sleep_time = datetime.datetime.strptime(
            self.user_data[group_id][user_id]["sleep_time"], "%Y-%m-%d %H:%M:%S"
        )
    ...
    # 进行早安并更新数据
    def morning_and_update(
        self, now_time: datetime.datetime, group_id: str, user_id: str
        ...
        if day == 0:
            in_sleep_tmp = (
                str(int(hour)) + "时" + str(int(minute)) + "分" + str(int(second)) + "秒"
            )
        self.user_data[group_id][user_id]["get_up_time"] = now_time.strftime(
            "%Y-%m-%d %H:%M:%S"
        )
        self.user_data[group_id][user_id]["morning_count"] = (
            int(self.user_data[group_id][user_id]["morning_count"]) + 1
        )
        # 判断是今天第几个起床的
        self.user_data[group_id]["today_count"]["morning"] = (
            int(self.user_data[group_id]["today_count"]["morning"]) + 1
        )
        self.save_data()
    ...
    def get_morning_msg(self, sex_str: str, event: GroupMessageEvent) -> str:
        ...
        if self.config["morning"]["get_up_intime"]["enable"]:
            ...
            if not MorningManager.judge_mor_time(
                early_time_tmp, late_time_tmp, now_time
            ):
        ...
        if user_id in self.user_data[group_id].keys():
            ...
            if (
                not self.config["morning"]["multi_get_up"]["enable"]
                and self.user_data[group_id][user_id]["get_up_time"] != 0
            ):
    ...
    def night_and_update(
        ...
        # 若有就更新数据
            ...
            self.user_data[group_id][user_id]["sleep_time"] = now_time.strftime(
                "%Y-%m-%d %H:%M:%S"
            )
            self.user_data[group_id][user_id]["night_count"] = (
                int(self.user_data[group_id][user_id]["night_count"]) + 1
            )
        ...
        # 判断是今天第几个睡觉的
        self.user_data[group_id]["today_count"]["night"] = (
            int(self.user_data[group_id]["today_count"]["night"]) + 1
        )
        self.save_data()
    ...
    def get_night_msg(self, sex_str: str, event: GroupMessageEvent) -> str:
        ...
        if self.config["night"]["sleep_intime"]["enable"]:
            ...
            if not MorningManager.judge_sle_time(
                early_time_tmp, late_time_tmp, now_time
            ):
        ...
        if user_id in self.user_data[group_id].keys():
            ...
            if (
                not self.config["night"]["deep_sleep"]["enable"]
                and self.user_data[group_id][user_id]["get_up_time"] != 0
            ):
